...
class Controller
{
    ...
    public Controller(Console console, InputQueue inputQueue, OptionsModel optionsModel) {
    ...
    private void RunUntilExit() {
        ...
        while (true) {
	    //Thread.Sleep(100);
            var evt = inputQueue.WaitForEvent();
            if (evt.keyPress != null) {
                onKeyPress(evt.keyPress.Value);
